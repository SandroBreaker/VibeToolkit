### <system_instruction>
Para executar o VibeToolkit e gerar um "AI Context Document" de altíssima densidade, siga os passos abaixo:

1. Clone o repositório do VibeToolkit e navegue até a pasta raiz do projeto.
2. Instale as dependências necessárias executando o comando `npm install`.
3. Configure sua chave de API da Groq no arquivo `.env`.
4. Execute o script `project-bundler.ps1` como administrador para configurar as permissões do PowerShell e integrar o menu ao botão direito do mouse.
5. Selecione o modo de extração desejado (BUNDLER, BLUEPRINT ou SELECTIVE) e siga as instruções para gerar o arquivo de contexto.

### # AI PROJECT CONTEXT BRIEFING

## 1. **Tech Stack**
O VibeToolkit utiliza as seguintes tecnologias:
* Node.js
* PowerShell
* Groq (Llama 3.3 70B)
* TypeScript
...
* `groq-agent.ts`: script responsável por interagir com a API da Groq
* `package.json`: arquivo de configuração do projeto
* `project-bundler.ps1`: script responsável por configurar as permissões do PowerShell e integrar o menu ao botão direito do mouse
* `README.md`: arquivo de documentação do projeto
* `setup-menu.ps1`: script responsável por configurar as permissões do PowerShell e integrar o menu ao botão direito do mouse
* `tsconfig.json`: arquivo de configuração do TypeScript

## 3. **Core Domains & Contracts**
O VibeToolkit utiliza os seguintes domínios e contratos:
* `GroqRequestParams`: interface que define os parâmetros da requisição à API da Groq
* `GroqService`: classe responsável por interagir com a API da Groq

## 4. **System Instruction**
A instrução do sistema é a seguinte:
...
## 5. **Blueprint Issues**
O VibeToolkit pode apresentar os seguintes problemas:
* Falha na API da Groq
* Erros de sintaxe no código
* Problemas de permissão ao executar o script do PowerShell

Esses problemas podem ser resolvidos verificando a documentação da API da Groq, revisando o código e executando o script do PowerShell como administrador.
