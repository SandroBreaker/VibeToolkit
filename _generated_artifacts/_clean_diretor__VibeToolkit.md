...
function Get-SentinelBundleModeTone {
    ...
    switch ($BundleModeValue) {
        ...
        'cleanExport' { return 'Muted' } 
        'clean_export' { return 'Muted' }
...
function Get-SentinelBundleModeHintLines {
    return @(
        'Full lê tudo. Blueprint prioriza estrutura. Sniper abre recorte manual.',
        'TXT Export gera ZIP. Clean Export gera MD limpo sem orquestração. Digite 1-5.'
    )
}
...
function Test-IsGeneratedArtifactFileName {
    ...
    $patterns = @(
        '^_(?:bundle|blueprint|manual|txt_export|clean)_(?:diretor|executor)(?:_[A-Za-z0-9\-]+)?__.*\.(?:md|json)$',
        '^_meta-prompt_(?:bundle|blueprint|manual)_(?:diretor|executor)(?:_[A-Za-z0-9\-]+)?__.*\.(?:md|json)$',
...
function Resolve-ExtractionModeFromBundleMode {
    ...
    switch ($ModeValue) {
        ...
        'cleanExport' { return 'clean_export' }
        default { return 'full' }
    }
}
...
function Resolve-DocumentModeFromExtractionMode {
    ...
    if ($ExtractionMode -eq 'txt_export') { return 'txt_export' }
    if ($ExtractionMode -eq 'clean_export') { return 'clean_export' }
    if ($ExtractionMode -eq 'blueprint') { return 'blueprint' }
...
function Get-SentinelConsoleMultiSelectHintLines {
    ...
    $defaultHints = @(
        ...
        '↑↓ NAVEGAR ESPACO MARCAR/DESMARCAR F FILTRAR A TODOS N NENHUM ENTER CONFIRMAR Q CANCELAR',
        '↑↓ NAVEGAR ESPAÇO MARCAR/DESMARCAR F FILTRAR A TODOS N NENHUM ENTER CONFIRMAR Q CANCELAR'
    )

    if ($defaultHints -contains $normalizedHint) {
        return @(
            '  ↑↓ mover · PgUp/PgDn página · Home/End extremos',
            '  Espaço marca · Enter confirma · A todos · N nenhum · F filtra · Q/Esc sai'
        )
    }
...
function Invoke-ConsoleMultiSelect {
    param(
        ...
        [string]$Hint = '↑↓ Navegar   ESPACO Marcar/Desmarcar   F Filtrar   A Todos   N Nenhum   ENTER Confirmar   Q Cancelar'
    )
    ...
    while ($true) {
        ...
        $visibleItems = if ([string]::IsNullOrWhiteSpace($filterText)) {
            $Items
        } else {
            @($Items | Where-Object { $_ -match [regex]::Escape($filterText) })
        }

        if ($visibleItems.Count -eq 0) {
            $currentIndex = 0
        } elseif ($currentIndex -ge $visibleItems.Count) {
            $currentIndex = [Math]::Max(0, $visibleItems.Count - 1)
        }
        ...
        if (-not [string]::IsNullOrWhiteSpace($filterText)) {
            Write-Host ("  [Filtro Ativo: '$filterText']`n") -ForegroundColor Magenta
        }

        $endIndex = [Math]::Min($visibleItems.Count - 1, $offset + $pageSize - 1)
        if ($offset -gt 0) {
        ...
        if ($visibleItems.Count -gt 0) {
            ...
            Write-Host '  (Nenhum item corresponde ao filtro atual)' -ForegroundColor DarkGray
        }

        if ($endIndex -lt ($visibleItems.Count - 1) -and $visibleItems.Count -gt 0) {
            Write-Host ('  … itens seguintes: {0}' -f ($visibleItems.Count - $endIndex - 1)) -ForegroundColor DarkGray
        }

        Write-Host ''
        Write-SentinelDivider -Tone 'Muted'
        Write-Host ('  Marcados : {0}/{1}' -f $selected.Count, $Items.Count) -ForegroundColor Yellow
        $displayStart = if ($visibleItems.Count) { $offset + 1 } else { 0 }
        $displayEnd = if ($visibleItems.Count) { $endIndex + 1 } else { 0 }
        $displayCurrent = if ($visibleItems.Count) { $currentIndex + 1 } else { 0 }
        Write-Host ('  Exibindo : {0}-{1} de {2}' -f $displayStart, $displayEnd, $visibleItems.Count) -ForegroundColor DarkGray
        Write-Host ('  Atual    : {0} de {1}' -f $displayCurrent, $visibleItems.Count) -ForegroundColor DarkGray
        ...
        switch ($keyInfo.Key) {
            ...
            'DownArrow' {
                if ($currentIndex -lt ($visibleItems.Count - 1)) { $currentIndex++ }
                continue
            }
            'PageUp' {
            ...
            'PageDown' {
                $currentIndex = [Math]::Min($visibleItems.Count - 1, $currentIndex + $pageSize)
        ...
        switch ($keyChar) {
            'f' {
                Write-Host ''
                Write-Host '  --- Filtro de Busca ---' -ForegroundColor Cyan
                Write-Host '  Digite o texto para filtrar (vazio para limpar): ' -NoNewline -ForegroundColor DarkGray
                $newFilter = Read-Host
...
function Resolve-SniperRequestedPaths {
    ...
    if (Test-InteractiveConsoleSelectionSupported) {
        return @(
            Invoke-ConsoleMultiSelect -Items $relativeOptions -Title 'SENTINEL HEADLESS — Seleção Sniper' -Hint '↑↓ Navegar   ESPAÇO Marcar/Desmarcar   F Filtrar   A Todos   N Nenhum   ENTER Confirmar   Q Cancelar'
        )
    }
...
function Get-VibeArtifactModeLabel {
    ...
    switch -Regex ($ExtractionMode) {
        ...
        '(?i)clean_export|cleanExport|^5$' { return 'clean' }
...
function Write-LocalExecutionMeta {
    ...
    $resolvedResultMetaPath = if ([string]::IsNullOrWhiteSpace($ResultMetaPath)) {
        ...
        $logsDir = Join-Path $baseDir '_generated_logs'
        Join-Path $logsDir (Get-ResultMetaOutputFileName -ProjectNameValue $ProjectNameValue -RouteMode $RouteMode -ExtractionMode $ExtractionMode)
    }
    else {
        $ResultMetaPath
    }
...
function Resolve-BundleMode {
    ...
    if ($BundleMode -in @('full', 'blueprint', 'sniper', 'txtExport', 'txt_export', 'cleanExport', 'clean_export')) {
        if ($BundleMode -eq 'txt_export') { return 'txtExport' }
        if ($BundleMode -eq 'clean_export') { return 'cleanExport' }
        return $BundleMode
    }

    if ($NonInteractive) {
        throw 'Modo de extração obrigatório em execução não interativa. Use -BundleMode full, blueprint, sniper, txtExport ou cleanExport.'
    }
    ...
    Write-SentinelMenuOptions -Options @(
        ...
        (New-SentinelMenuOptionLine -Label 'Clean Export' -Description 'arquivo MD puro, sem metadados ou regras' -LabelWidth 14)
    )
    Write-SentinelHintLines -Lines (Get-SentinelBundleModeHintLines)

    $resolved = $null
    while ($null -eq $resolved) {
        $inp = (Read-Host '  Escolha a extração [1-5]').Trim()
        switch ($inp) {
            '1' { $resolved = 'full' }
            '2' { $resolved = 'blueprint' }
            '3' { $resolved = 'sniper' }
            '4' { $resolved = 'txtExport' }
            '5' { $resolved = 'cleanExport' }
            default { Write-SentinelInvalidSelection -Expected '1, 2, 3, 4 ou 5' }
        }
    }
...
$script:AllowedExtensions = @(
    ...
    '.gradle', '.sln', '.csproj', '.vbproj', '.fsproj', '.vcxproj', '.filters', '.reg', '.vbs', '.txt'
)
...
try {
    ...
    $headerContent = ''
    if ($currentExtractionMode -ne 'clean_export') {
    ...
    $finalContent = if ([string]::IsNullOrWhiteSpace($headerContent)) { "" } else { $headerContent + "`n`n" }
    $blueprintIssues = @()

    if ($choice -eq '1' -or $choice -eq '3' -or $choice -eq '5') {
        if ($choice -eq '1') {
        ...
        elseif ($choice -eq '5') {
            $sourceArtifactFileName = Get-VibeArtifactFileName -ProjectNameValue $projectName -ExtractionMode $currentExtractionMode -RouteMode $resolvedRouteMode
            $headerTitle = 'MODO CLEAN EXPORT'
            Write-UILog -Message 'Iniciando Modo Clean Export (Gerando conteúdo puro)...' -Color $ThemeCyan
        }
        else {
            $sourceArtifactFileName = Get-VibeArtifactFileName -ProjectNameValue $projectName -ExtractionMode $currentExtractionMode -RouteMode $resolvedRouteMode
...
'@.Trim() -Language 'text')
    ...
    $logsDir = Join-Path $script:EffectiveOutputDirectory '_generated_logs'
    $resultMetaPath = Join-Path $logsDir ([System.IO.Path]::GetFileNameWithoutExtension($finalOutputPath) + '.json')
